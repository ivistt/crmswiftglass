...
function renderManagerOrderCardMeta(order) {
  ...
  const groups = [
    ...
    [
      ...
      { label: 'Доп заметка', value: order.extraNote || '—' },
      { label: 'Вин-код', value: order.vin || '—' },
    ],
    [
      { label: 'Склад', value: order.warehouse || '—' },
      { label: 'Код склада', value: order.warehouseCode || '—' },
    ],
    [
      { label: '', value: String(order.notes || '—').trim() || '—', strong: true },
    ],
    [
      { label: 'Менеджер', value: getWorkerDisplayName(order.manager) || '—' },
    ],
  ];
